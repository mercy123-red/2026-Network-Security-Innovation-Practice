#include <stddef.h>
#include "sm4_internal.h"

const uint8_t SM4_SBOX[256] = {

};

static const uint32_t FK[4] = {
    0xa3b1bac6U,0x56aa3350U,0x677d9197U,0xb27022dcU
};

static const uint32_t CK[32] = {
0x00070e15U,0x1c232a31U,0x383f464dU,0x545b6269U,
0x70777e85U,0x8c939aa1U,0xa8afb6bdU,0xc4cbd2d9U,
0xe0e7eef5U,0xfc030a11U,0x181f262dU,0x343b4249U,
0x50575e65U,0x6c737a81U,0x888f969dU,0xa4abb2b9U,
0xc0c7ced5U,0xdce3eaf1U,0xf8ff060dU,0x141b2229U,
0x30373e45U,0x4c535a61U,0x686f767dU,0x848b9299U,
0xa0a7aeb5U,0xbcc3cad1U,0xd8dfe6edU,0xf4fb0209U,
0x10171e25U,0x2c333a41U,0x484f565dU,0x646b7279U
};

uint32_t sm4_tau(uint32_t x) {
    return ((uint32_t)SM4_SBOX[(x >> 24) & 0xffU] << 24) |
           ((uint32_t)SM4_SBOX[(x >> 16) & 0xffU] << 16) |
           ((uint32_t)SM4_SBOX[(x >> 8) & 0xffU] << 8) |
           (uint32_t)SM4_SBOX[x & 0xffU];
}

uint32_t sm4_t(uint32_t x) {
    const uint32_t b = sm4_tau(x);
    return b ^ sm4_rotl32(b,2) ^ sm4_rotl32(b,10) ^
           sm4_rotl32(b,18) ^ sm4_rotl32(b,24);
}

uint32_t sm4_t_key(uint32_t x) {
    const uint32_t b = sm4_tau(x);
    return b ^ sm4_rotl32(b,13) ^ sm4_rotl32(b,23);
}

int sm4_init(sm4_ctx *ctx, const uint8_t key[16]) {
    uint32_t k0, k1, k2, k3;
    if (ctx == NULL || key == NULL) {
        return -1;
    }
    k0 = sm4_load_be32(key) ^ FK[0];
    k1 = sm4_load_be32(key + 4) ^ FK[1];
    k2 = sm4_load_be32(key + 8) ^ FK[2];
    k3 = sm4_load_be32(key + 12) ^ FK[3];

    for (unsigned i = 0; i < 32; ++i) {
        const uint32_t n = k0 ^ sm4_t_key(k1 ^ k2 ^ k3 ^ CK[i]);
        ctx->rk_enc[i] = n;
        k0 = k1; k1 = k2; k2 = k3; k3 = n;
    }
    for (unsigned i = 0; i < 32; ++i) {
        ctx->rk_dec[i] = ctx->rk_enc[31U - i];
    }
    sm4_init_ttables();
    return 0;
}
